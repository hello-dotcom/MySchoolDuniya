...
class Notices extends Component {
	...
	setCollapsible=(id)=>{
		...
		if(coll[id].style.display==="block")
			coll[id].style.display="none";
		else	
			coll[id].style.display="block";
	}
					...
					this.state.notices.map((item,index)=>{
						return (
							<div className="noticeItem">
								<button  type="button"  onClick={()=>this.setCollapsible(index)} className="collapsible">{item.notification_no}:-{item.heading}</button>
								<div className="content">
									<p>{item.message}</p>
									<div className="flex justify-content-between">
										{(item.filename!=="empty")?<p onClick={()=>this.downloadFile(item.filename,item.notification_no)} className="fileItem">File</p>:null}
										{(localStorage.getItem('token') && localStorage.getItem('role')==='Admin')?<svg onClick={()=>this.deleteNotice(item.notification_no)} xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="red" class="bi bi-trash dustbin float-right" viewBox="0 0 16 16">
