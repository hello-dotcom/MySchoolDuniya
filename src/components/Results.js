	...
	submitResults = () => {
		...
		if (this.state.marks_list.length === 0) {
			for (i = 0; i < abc.length; i++) {
				marks.push({
					subject_id: this.state.subject_list[i].subject_id,
					marks: abc[i].value,
				});
			}
		} else {
			for (i = 0; i < abc.length; i++) {
				marks.push({
					subject_id: this.state.marks_list[i].subject_id,
					marks: abc[i].value,
				});
			}
		}
				...
				{this.state.stage === 1 ? (
					<div>
						{this.state.classList.map((item, index) => {
							return (
								<button
									key={index}
									onClick={() => this.getStudents(item.class_id)}
								>
									{item.class_id}
								</button>
							);
					...
					<div>
						<Table striped bordered hover>
							...
							<tbody>
								{this.state.stdList &&
									this.state.stdList.map((item, index) => {
										return (
											<tr key={index}>
												<td>{item.id}</td>
												<td>{item.name}</td>
												...
												<td>
													<button
														...
														onClick={() => this.gotoResults(item.id, item.name)}
													>
														Update Result
													</button>
												</td>
												<td>
													<button
														className="btn-container"
														onClick={() => this.ShowResults(item.id, item.name)}
													>
														Show Result
													</button>
												</td>
							...
							<Table striped bordered hover>
								...
								<tbody>
									{this.state.marks_list.length === 0
										? this.state.subject_list.map((item, index) => {
												return (
													<tr key={index}>
